...
describe('AuthService class', () => {
  it('method signIn should sign in a user with valid credentials', async () => {
    const user = {
  ...
  it('method signIn should throw an error for invalid credentials', async () => {
    const user = {
  ...
  it('method registerUser should register a user with valid details', async () => {
    const user = {
      name: 'Test User',
      email: 'test@example.com',
      password: 'password123',
  ...
  it('method registerUser should throw an error for registration failure', async () => {
    const user = {
      name: 'Invalid User',
      email: 'invalid@example.com',
      password: 'invalidpassword',
    ...
    await expect(AuthService.registerUser(user)).rejects.toThrow(
      ERROR.FIREBASE.MESSAGE
    );
  });

  it('method checkEmail should return true for existing email', async () => {
  ...
  it('method checkEmail should return false for non-existing email', async () => {
  ...
  it('method  checkEmail should throw an error for check email failure', async () => {
  ...
  it('method logout should call signOut method', () => {
    AuthService.logout();
